...
unsigned char
	...
	tmr_on;		// ==1 - таймер включен

unsigned char write_SPI(unsigned char);
void hands(){
	unsigned char i;
	write_SPI(10); // инициализация
	write_SPI(0);
/*	write_SPI(128);
	for(i=0; i<20; i++)
		write_SPI(0);*/
...
// получение адреса
	...
	SSPCON = 0x32; // (00110010) - включить SPI, высокий уровень CLK (CKP=1), частота Fosc/64
	// SSPSTAT: | SMP | CKE | - | - | - | - | - | BF |
...
unsigned char write_SPI(unsigned char byte){
	...
	TMR1IF = 0;
	if(tmr_on){
		T1CON = 0; // отключаем таймер
		TMR1IE = 0;
	}
	SSPBUF = byte;
	while(!SSPIF); // ждем окончания передачи
	ans = SSPBUF; // регистрируем полученный байт
	SSPIF = 0;
	SSPIE = 1;
	if(tmr_on){
		TMR1H = T1H; TMR1L = T1L;
		T1CON = 0x31; // запускаем таймер
		TMR1IE = 1;
	}
	return ans;
}
...
void  on_interrupt() __interrupt 0{ // обработка прерываний
	if(RCIF == 1){	// поступило прерывание от USART
		...
		switch(cmd){
			...
			case SPEED1:		d_H = 0x6D; d_L = 0x84; timer1set(); break; // 0.3с
			case SPEED2:		d_H = 0xE7; d_L = 0x96; timer1set(); break; // 0.05с
			case SPEED3:		d_H = 0xFB; d_L = 0x1E; timer1set(); break; // 0.01с
	...
	if(TMR1IF == 1 && tmr_on){ // поступило прерывание от таймера
		write_SPI(4); // посылаем сигнал стоп и перезапускаем таймер
		write_SPI(0);
	}
}
